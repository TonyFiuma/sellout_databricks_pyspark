...
OUTPUT_COLUMN_TYPES = {
    ...
    "product_family_level_1": "string",
    "brand_company": "string",
    "item_code": "string",
    "channel_raw": "string",
    "customer": "string",
    "sales_country": "string",
    "is_saliva_ejector": "string",
    "gross_sales_currency": "string",
    "quantity": "double",
    "gross_sales": "double",
}
OUTPUT_COLUMNS = list(OUTPUT_COLUMN_TYPES)
FORCED_NULL_COLUMNS = {"product_family_level_1"}
...
def output_expression(target_column: str, expressions: list):
    if len(expressions) == 1:
        return expressions[0].alias(target_column)

    print(f"Coalescing {len(expressions)} source columns into {target_column}")
    return F.coalesce(*expressions).alias(target_column)
...
for source_column in source_df.columns:
    ...
    if target_column not in OUTPUT_COLUMNS or target_column in FORCED_NULL_COLUMNS:
        continue

    spark_type = spark_type_for(mapping_row["data_types_found"])
    expression = F.col(quote_column(source_column)).cast(spark_type)

    target_columns.setdefault(target_column, []).append(
        expression
    )
...
for target_column in OUTPUT_COLUMNS:
    if target_column == "date":
        selected_columns.append(build_date_column(source_df.columns, field_mapping))
    elif target_column in FORCED_NULL_COLUMNS:
        selected_columns.append(
            F.lit(None).cast(OUTPUT_COLUMN_TYPES[target_column]).alias(target_column)
        )
    elif target_column in target_columns:
        expressions = target_columns[target_column]
        selected_columns.append(output_expression(target_column, expressions))
    else:
        missing_output_columns.append(target_column)
        selected_columns.append(
            F.lit(None).cast(OUTPUT_COLUMN_TYPES[target_column]).alias(target_column)
        )
...
if missing_output_columns:
    print(
        "Output columns not found in source mapping; writing them as NULL: "
